...
export default function(socket: Socket, handlerCallback) {
  ...
  function resetSocket(graceful = false, nextContent = Buffer.alloc(0)) {
    if (!graceful) console.error("Socket was reset because the message was damaged");
  ...
  socket.on("data", message => {
    ...
    const isContentPending = (contentReceived < contentLength);

    console.log(typeof contentLength, typeof contentReceived);
    ...
    if (!isContentPending) {
      ...
      if (contentReceived > contentLength) {
        const nextMessage = currentBuffer.slice(contentStart + contentLength);
